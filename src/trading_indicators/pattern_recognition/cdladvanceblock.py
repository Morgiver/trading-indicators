"""
CDLADVANCEBLOCK - Advance Block Candlestick Pattern

Advance Block is a bearish reversal pattern consisting of three white candles
with progressively smaller bodies and longer upper shadows.

Returns:
- 0: Pattern not detected
- 100: Bullish Advance Block
- -100: Bearish Advance Block
"""
...
class CDLADVANCEBLOCK(BaseIndicator):
    """
    CDLADVANCEBLOCK - Advance Block Candlestick Pattern

    A bearish reversal pattern with three consecutive white candles showing
    diminishing momentum, signaling potential exhaustion of upward movement.

    Usage:
        # Auto-sync mode
        advance_block = CDLADVANCEBLOCK(frame=frame)
        if advance_block.is_bearish():
            print("Bearish Advance Block detected")

        # Utility mode
        pattern = CDLADVANCEBLOCK.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLADVANCEBLOCK', max_periods=None):
        """
        Initialize CDLADVANCEBLOCK indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLADVANCEBLOCK using TA-Lib (utility mode).
        ...
        return talib.CDLADVANCEBLOCK(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLADVANCEBLOCK for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Advance Block pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Advance Block pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Advance Block pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
