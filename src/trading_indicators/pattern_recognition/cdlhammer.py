...
Hammer is a bullish reversal pattern with a small body at the upper end
and a long lower shadow, appearing after a downtrend.
...
class CDLHAMMER(BaseIndicator):
    """
    CDLHAMMER - Hammer Candlestick Pattern

    A bullish reversal pattern with a small body and long lower shadow,
    indicating rejection of lower prices and potential upward reversal.
    ...
    Usage:
        ...
        # Utility mode
        pattern = CDLHAMMER.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLHAMMER', max_periods=None):
        """
        Initialize CDLHAMMER indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLHAMMER using TA-Lib (utility mode).
    ...
    def calculate(self, period):
        """Calculate CDLHAMMER for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Hammer pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Hammer pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Hammer pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
