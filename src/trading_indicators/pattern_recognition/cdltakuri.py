"""
CDLTAKURI - Takuri Candlestick Pattern

Takuri is a bullish reversal pattern similar to Dragonfly Doji but with
a very long lower shadow, indicating strong rejection of lower prices.
...
class CDLTAKURI(BaseIndicator):
    """
    CDLTAKURI - Takuri Candlestick Pattern

    A bullish reversal pattern with a Dragonfly Doji featuring a very long
    lower shadow, showing strong buying pressure and price rejection.

    Usage:
        # Auto-sync mode
        takuri = CDLTAKURI(frame=frame)
        if takuri.is_bullish():
            print("Bullish Takuri detected")

        # Utility mode
        pattern = CDLTAKURI.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLTAKURI', max_periods=None):
        """
        Initialize CDLTAKURI indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLTAKURI using TA-Lib (utility mode).
        ...
        return talib.CDLTAKURI(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLTAKURI for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Takuri pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Takuri pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Takuri pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
