"""
CDLTRISTAR - Tristar Pattern Candlestick Pattern

Tristar Pattern is a rare reversal pattern consisting of three Doji candles,
with the middle Doji gapping away from the other two.

Returns:
- 0: Pattern not detected
- 100: Bullish Tristar Pattern
- -100: Bearish Tristar Pattern
"""
...
class CDLTRISTAR(BaseIndicator):
    """
    CDLTRISTAR - Tristar Pattern Candlestick Pattern

    A rare and significant reversal pattern with three Doji candles showing
    extreme indecision before a major trend reversal.

    Usage:
        # Auto-sync mode
        tristar = CDLTRISTAR(frame=frame)
        if tristar.is_bullish():
            print("Bullish Tristar Pattern detected")

        # Utility mode
        pattern = CDLTRISTAR.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLTRISTAR', max_periods=None):
        """
        Initialize CDLTRISTAR indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLTRISTAR using TA-Lib (utility mode).
        ...
        return talib.CDLTRISTAR(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLTRISTAR for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Tristar Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Tristar Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Tristar Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
