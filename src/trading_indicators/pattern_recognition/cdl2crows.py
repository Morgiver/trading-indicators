"""
CDL2CROWS - Two Crows Candlestick Pattern

Two Crows is a bearish reversal pattern that appears in an uptrend. It consists
of three candles where two black candles follow a long white candle, signaling
potential trend reversal.

Returns:
- 0: Pattern not detected
- 100: Bullish Two Crows
...
class CDL2CROWS(BaseIndicator):
    """
    CDL2CROWS - Two Crows Candlestick Pattern

    A bearish reversal pattern appearing in uptrends, consisting of a long white
    candle followed by two black candles that gap up and then close lower.

    Usage:
        # Auto-sync mode
        two_crows = CDL2CROWS(frame=frame)
        if two_crows.is_bearish():
            print("Bearish Two Crows detected")

        # Utility mode
        pattern = CDL2CROWS.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDL2CROWS', max_periods=None):
        """
        Initialize CDL2CROWS indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDL2CROWS using TA-Lib (utility mode).
        ...
        return talib.CDL2CROWS(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDL2CROWS for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Two Crows pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Two Crows pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Two Crows pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
