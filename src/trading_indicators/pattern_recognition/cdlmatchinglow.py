"""
CDLMATCHINGLOW - Matching Low Candlestick Pattern

Matching Low is a bullish reversal pattern with two black candles that
have identical lows, appearing after a downtrend.

Returns:
- 0: Pattern not detected
- 100: Bullish Matching Low
- -100: Bearish Matching Low
"""
...
class CDLMATCHINGLOW(BaseIndicator):
    """
    CDLMATCHINGLOW - Matching Low Candlestick Pattern

    A bullish reversal pattern with two consecutive black candles sharing
    the same low, indicating support and potential upward reversal.

    Usage:
        # Auto-sync mode
        matching_low = CDLMATCHINGLOW(frame=frame)
        if matching_low.is_bullish():
            print("Bullish Matching Low detected")

        # Utility mode
        pattern = CDLMATCHINGLOW.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLMATCHINGLOW', max_periods=None):
        """
        Initialize CDLMATCHINGLOW indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLMATCHINGLOW using TA-Lib (utility mode).
        ...
        return talib.CDLMATCHINGLOW(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLMATCHINGLOW for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Matching Low pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Matching Low pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Matching Low pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
