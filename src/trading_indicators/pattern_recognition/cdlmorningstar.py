"""
CDLMORNINGSTAR - Morning Star Candlestick Pattern

Morning Star is a bullish reversal pattern consisting of a black candle,
a small-bodied candle that gaps down, and a white candle that closes into the first.

Returns:
- 0: Pattern not detected
- 100: Bullish Morning Star
- -100: Bearish Morning Star
"""
...
class CDLMORNINGSTAR(BaseIndicator):
    """
    CDLMORNINGSTAR - Morning Star Candlestick Pattern

    A bullish reversal pattern with three candles: black candle, small gapped-down
    candle (star), and white candle closing into first candle's body.

    Usage:
        # Auto-sync mode
        morning_star = CDLMORNINGSTAR(frame=frame)
        if morning_star.is_bullish():
            print("Bullish Morning Star detected")

        # Utility mode
        pattern = CDLMORNINGSTAR.compute(open_prices, high_prices, low_prices, close_prices, penetration=0.3)
    """

    def __init__(self, frame=None, column_name='CDLMORNINGSTAR', max_periods=None, penetration=0.3):
        """
        Initialize CDLMORNINGSTAR indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices, penetration=0.3):
        """
        Compute CDLMORNINGSTAR using TA-Lib (utility mode).
        ...
        return talib.CDLMORNINGSTAR(open_prices, high_prices, low_prices, close_prices, penetration=penetration)

    def calculate(self, period):
        """Calculate CDLMORNINGSTAR for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Morning Star pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Morning Star pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Morning Star pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
