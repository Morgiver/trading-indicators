"""
CDL3STARSINSOUTH - Three Stars In The South Candlestick Pattern

Three Stars In The South is a bullish reversal pattern consisting of three
consecutive black candles with diminishing lower shadows, signaling exhaustion.

Returns:
- 0: Pattern not detected
- 100: Bullish Three Stars In The South
- -100: Bearish Three Stars In The South
"""
...
class CDL3STARSINSOUTH(BaseIndicator):
    """
    CDL3STARSINSOUTH - Three Stars In The South Candlestick Pattern

    A bullish reversal pattern with three black candles showing progressively
    diminishing lower shadows, indicating selling pressure exhaustion.

    Usage:
        # Auto-sync mode
        three_stars = CDL3STARSINSOUTH(frame=frame)
        if three_stars.is_bullish():
            print("Bullish Three Stars In The South detected")

        # Utility mode
        pattern = CDL3STARSINSOUTH.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDL3STARSINSOUTH', max_periods=None):
        """
        Initialize CDL3STARSINSOUTH indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDL3STARSINSOUTH using TA-Lib (utility mode).
        ...
        return talib.CDL3STARSINSOUTH(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDL3STARSINSOUTH for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Three Stars In The South pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Three Stars In The South pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Three Stars In The South pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
