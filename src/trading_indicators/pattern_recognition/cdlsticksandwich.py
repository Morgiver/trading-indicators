"""
CDLSTICKSANDWICH - Stick Sandwich Candlestick Pattern

Stick Sandwich is a bullish reversal pattern with two black candles
with identical closes sandwiching a white candle.

Returns:
- 0: Pattern not detected
- 100: Bullish Stick Sandwich
- -100: Bearish Stick Sandwich
"""
...
class CDLSTICKSANDWICH(BaseIndicator):
    """
    CDLSTICKSANDWICH - Stick Sandwich Candlestick Pattern

    A bullish reversal pattern with two black candles having identical
    closes sandwiching a white candle, signaling support and reversal.

    Usage:
        # Auto-sync mode
        stick_sandwich = CDLSTICKSANDWICH(frame=frame)
        if stick_sandwich.is_bullish():
            print("Bullish Stick Sandwich detected")

        # Utility mode
        pattern = CDLSTICKSANDWICH.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLSTICKSANDWICH', max_periods=None):
        """
        Initialize CDLSTICKSANDWICH indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLSTICKSANDWICH using TA-Lib (utility mode).
        ...
        return talib.CDLSTICKSANDWICH(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLSTICKSANDWICH for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Stick Sandwich pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Stick Sandwich pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Stick Sandwich pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
