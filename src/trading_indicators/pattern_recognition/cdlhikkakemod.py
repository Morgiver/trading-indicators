"""
CDLHIKKAKEMOD - Modified Hikkake Pattern Candlestick Pattern

Modified Hikkake Pattern is a variation of the Hikkake pattern with additional
confirmation candles, providing stronger signals.

Returns:
- 0: Pattern not detected
- 100: Bullish Modified Hikkake Pattern
- -100: Bearish Modified Hikkake Pattern
"""
...
class CDLHIKKAKEMOD(BaseIndicator):
    """
    CDLHIKKAKEMOD - Modified Hikkake Pattern Candlestick Pattern

    An enhanced version of the Hikkake pattern with additional confirmation,
    providing more reliable reversal or continuation signals.

    Usage:
        # Auto-sync mode
        hikkake_mod = CDLHIKKAKEMOD(frame=frame)
        if hikkake_mod.is_bullish():
            print("Bullish Modified Hikkake Pattern detected")

        # Utility mode
        pattern = CDLHIKKAKEMOD.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLHIKKAKEMOD', max_periods=None):
        """
        Initialize CDLHIKKAKEMOD indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLHIKKAKEMOD using TA-Lib (utility mode).
        ...
        return talib.CDLHIKKAKEMOD(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLHIKKAKEMOD for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Modified Hikkake Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Modified Hikkake Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Modified Hikkake Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
