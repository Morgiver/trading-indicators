"""
CDLKICKINGBYLENGTH - Kicking - bull/bear determined by the longer marubozu Candlestick Pattern

Kicking By Length is a variation of the Kicking pattern where the direction
is determined by the longer of the two Marubozu candles.
...
class CDLKICKINGBYLENGTH(BaseIndicator):
    """
    CDLKICKINGBYLENGTH - Kicking - bull/bear determined by the longer marubozu

    A Kicking pattern variation where the longer Marubozu determines the
    pattern direction, providing clearer directional signals.

    Usage:
        # Auto-sync mode
        kicking_by_length = CDLKICKINGBYLENGTH(frame=frame)
        if kicking_by_length.is_bullish():
            print("Bullish Kicking By Length detected")

        # Utility mode
        pattern = CDLKICKINGBYLENGTH.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLKICKINGBYLENGTH', max_periods=None):
        """
        Initialize CDLKICKINGBYLENGTH indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLKICKINGBYLENGTH using TA-Lib (utility mode).
        ...
        return talib.CDLKICKINGBYLENGTH(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLKICKINGBYLENGTH for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Kicking By Length pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Kicking By Length pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Kicking By Length pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
