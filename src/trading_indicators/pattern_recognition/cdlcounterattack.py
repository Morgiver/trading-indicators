"""
CDLCOUNTERATTACK - Counterattack Candlestick Pattern

Counterattack is a reversal pattern where opposite-colored candles have the
same closing price, indicating a potential trend reversal.

Returns:
- 0: Pattern not detected
- 100: Bullish Counterattack
- -100: Bearish Counterattack
"""
...
class CDLCOUNTERATTACK(BaseIndicator):
    """
    CDLCOUNTERATTACK - Counterattack Candlestick Pattern

    A two-candle reversal pattern where opposite-colored candles close at
    the same level, signaling a shift in market sentiment.

    Usage:
        # Auto-sync mode
        counterattack = CDLCOUNTERATTACK(frame=frame)
        if counterattack.is_bullish():
            print("Bullish Counterattack detected")

        # Utility mode
        pattern = CDLCOUNTERATTACK.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLCOUNTERATTACK', max_periods=None):
        """
        Initialize CDLCOUNTERATTACK indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLCOUNTERATTACK using TA-Lib (utility mode).
        ...
        return talib.CDLCOUNTERATTACK(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLCOUNTERATTACK for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Counterattack pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Counterattack pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Counterattack pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
