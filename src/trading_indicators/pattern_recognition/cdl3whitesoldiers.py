"""
CDL3WHITESOLDIERS - Three Advancing White Soldiers Candlestick Pattern

Three Advancing White Soldiers is a bullish reversal pattern with three
consecutive long white candles, each opening within the previous body.

Returns:
- 0: Pattern not detected
- 100: Bullish Three Advancing White Soldiers
- -100: Bearish Three Advancing White Soldiers
"""
...
class CDL3WHITESOLDIERS(BaseIndicator):
    """
    CDL3WHITESOLDIERS - Three Advancing White Soldiers Candlestick Pattern

    A strong bullish reversal pattern with three consecutive long white candles,
    each opening within the previous candle's body and closing progressively higher.

    Usage:
        # Auto-sync mode
        three_white_soldiers = CDL3WHITESOLDIERS(frame=frame)
        if three_white_soldiers.is_bullish():
            print("Bullish Three Advancing White Soldiers detected")

        # Utility mode
        pattern = CDL3WHITESOLDIERS.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDL3WHITESOLDIERS', max_periods=None):
        """
        Initialize CDL3WHITESOLDIERS indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDL3WHITESOLDIERS using TA-Lib (utility mode).
        ...
        return talib.CDL3WHITESOLDIERS(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDL3WHITESOLDIERS for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Three Advancing White Soldiers pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Three Advancing White Soldiers pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Three Advancing White Soldiers pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
