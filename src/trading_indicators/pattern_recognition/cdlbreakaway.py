"""
CDLBREAKAWAY - Breakaway Candlestick Pattern

Breakaway is a five-candle reversal pattern where a gap is followed by
three same-direction candles, then a final candle closes within the gap.

Returns:
- 0: Pattern not detected
- 100: Bullish Breakaway
- -100: Bearish Breakaway
"""
...
class CDLBREAKAWAY(BaseIndicator):
    """
    CDLBREAKAWAY - Breakaway Candlestick Pattern

    A five-candle reversal pattern featuring a gap followed by continuation
    candles, then a reversal candle that closes within the gap.

    Usage:
        # Auto-sync mode
        breakaway = CDLBREAKAWAY(frame=frame)
        if breakaway.is_bullish():
            print("Bullish Breakaway detected")

        # Utility mode
        pattern = CDLBREAKAWAY.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLBREAKAWAY', max_periods=None):
        """
        Initialize CDLBREAKAWAY indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLBREAKAWAY using TA-Lib (utility mode).
        ...
        return talib.CDLBREAKAWAY(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLBREAKAWAY for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Breakaway pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Breakaway pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Breakaway pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
