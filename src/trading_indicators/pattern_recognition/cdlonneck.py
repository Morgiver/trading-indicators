"""
CDLONNECK - On-Neck Pattern Candlestick Pattern

On-Neck Pattern is a bearish continuation pattern where a white candle closes
at the low of the previous black candle in a downtrend.

Returns:
- 0: Pattern not detected
- 100: Bullish On-Neck Pattern
...
class CDLONNECK(BaseIndicator):
    """
    CDLONNECK - On-Neck Pattern Candlestick Pattern

    A bearish continuation pattern with a white candle that closes at
    the low of the previous black candle, confirming downtrend continuation.

    Usage:
        # Auto-sync mode
        on_neck = CDLONNECK(frame=frame)
        if on_neck.is_bearish():
            print("Bearish On-Neck Pattern detected")

        # Utility mode
        pattern = CDLONNECK.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLONNECK', max_periods=None):
        """
        Initialize CDLONNECK indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLONNECK using TA-Lib (utility mode).
        ...
        return talib.CDLONNECK(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLONNECK for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish On-Neck Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish On-Neck Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any On-Neck Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
