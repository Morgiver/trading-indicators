"""
CDLSPINNINGTOP - Spinning Top Candlestick Pattern

Spinning Top is a candle with a small body and long upper and lower shadows,
indicating indecision and potential trend reversal or continuation.
...
class CDLSPINNINGTOP(BaseIndicator):
    """
    CDLSPINNINGTOP - Spinning Top Candlestick Pattern

    A candle with a small body and shadows on both sides, showing
    indecision between buyers and sellers.

    Usage:
        # Auto-sync mode
        spinning_top = CDLSPINNINGTOP(frame=frame)
        if spinning_top.is_detected():
            print("Spinning Top detected")

        # Utility mode
        pattern = CDLSPINNINGTOP.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLSPINNINGTOP', max_periods=None):
        """
        Initialize CDLSPINNINGTOP indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLSPINNINGTOP using TA-Lib (utility mode).
        ...
        return talib.CDLSPINNINGTOP(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLSPINNINGTOP for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Spinning Top pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Spinning Top pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Spinning Top pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
