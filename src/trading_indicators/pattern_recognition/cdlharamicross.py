"""
CDLHARAMICROSS - Harami Cross Pattern Candlestick Pattern

Harami Cross Pattern is a reversal pattern where a Doji is contained
within the body of the previous larger candle, signaling strong reversal.

Returns:
- 0: Pattern not detected
- 100: Bullish Harami Cross Pattern
- -100: Bearish Harami Cross Pattern
"""
...
class CDLHARAMICROSS(BaseIndicator):
    """
    CDLHARAMICROSS - Harami Cross Pattern Candlestick Pattern

    A strong reversal pattern with a Doji contained within the previous
    larger candle's body, indicating significant indecision and reversal potential.

    Usage:
        # Auto-sync mode
        harami_cross = CDLHARAMICROSS(frame=frame)
        if harami_cross.is_bullish():
            print("Bullish Harami Cross Pattern detected")

        # Utility mode
        pattern = CDLHARAMICROSS.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLHARAMICROSS', max_periods=None):
        """
        Initialize CDLHARAMICROSS indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLHARAMICROSS using TA-Lib (utility mode).
        ...
        return talib.CDLHARAMICROSS(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLHARAMICROSS for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Harami Cross Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Harami Cross Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Harami Cross Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
