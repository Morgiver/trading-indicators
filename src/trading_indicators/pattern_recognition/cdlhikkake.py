"""
CDLHIKKAKE - Hikkake Pattern Candlestick Pattern

Hikkake Pattern is a complex reversal/continuation pattern involving an inside
bar followed by a false breakout and then a true breakout.

Returns:
- 0: Pattern not detected
- 100: Bullish Hikkake Pattern
- -100: Bearish Hikkake Pattern
"""
...
class CDLHIKKAKE(BaseIndicator):
    """
    CDLHIKKAKE - Hikkake Pattern Candlestick Pattern

    A complex pattern with an inside bar, false breakout, and subsequent
    true breakout, signaling potential trend reversal or continuation.

    Usage:
        # Auto-sync mode
        hikkake = CDLHIKKAKE(frame=frame)
        if hikkake.is_bullish():
            print("Bullish Hikkake Pattern detected")

        # Utility mode
        pattern = CDLHIKKAKE.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLHIKKAKE', max_periods=None):
        """
        Initialize CDLHIKKAKE indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLHIKKAKE using TA-Lib (utility mode).
        ...
        return talib.CDLHIKKAKE(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLHIKKAKE for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Hikkake Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Hikkake Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Hikkake Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
