"""
CDLSHORTLINE - Short Line Candle Candlestick Pattern

Short Line Candle is a single candle with a short body, indicating
indecision or consolidation in the market.

Returns:
- 0: Pattern not detected
- 100: Bullish Short Line Candle
- -100: Bearish Short Line Candle
"""
...
class CDLSHORTLINE(BaseIndicator):
    """
    CDLSHORTLINE - Short Line Candle Candlestick Pattern

    A candle with a short body showing limited price movement and
    indecision, often indicating consolidation or pause in trend.

    Usage:
        # Auto-sync mode
        short_line = CDLSHORTLINE(frame=frame)
        if short_line.is_detected():
            print("Short Line Candle detected")

        # Utility mode
        pattern = CDLSHORTLINE.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLSHORTLINE', max_periods=None):
        """
        Initialize CDLSHORTLINE indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLSHORTLINE using TA-Lib (utility mode).
        ...
        return talib.CDLSHORTLINE(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLSHORTLINE for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Short Line Candle pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Short Line Candle pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Short Line Candle pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
