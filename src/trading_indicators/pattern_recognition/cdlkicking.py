"""
CDLKICKING - Kicking Candlestick Pattern

Kicking is a strong reversal pattern with two Marubozu candles of opposite
color that gap away from each other, indicating powerful trend reversal.
...
class CDLKICKING(BaseIndicator):
    """
    CDLKICKING - Kicking Candlestick Pattern

    A powerful reversal pattern with two opposite-colored Marubozu candles
    that gap, signaling a strong shift in market sentiment.

    Usage:
        # Auto-sync mode
        kicking = CDLKICKING(frame=frame)
        if kicking.is_bullish():
            print("Bullish Kicking detected")

        # Utility mode
        pattern = CDLKICKING.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLKICKING', max_periods=None):
        """
        Initialize CDLKICKING indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLKICKING using TA-Lib (utility mode).
        ...
        return talib.CDLKICKING(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLKICKING for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Kicking pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Kicking pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Kicking pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
