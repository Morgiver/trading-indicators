"""
CDLMARUBOZU - Marubozu Candlestick Pattern

Marubozu is a candle with no shadows, where the open and close are at the
extremes, indicating strong directional momentum and conviction.
...
class CDLMARUBOZU(BaseIndicator):
    """
    CDLMARUBOZU - Marubozu Candlestick Pattern

    A candle with no shadows and full body from open to close, showing
    very strong momentum and conviction in the market direction.

    Usage:
        # Auto-sync mode
        marubozu = CDLMARUBOZU(frame=frame)
        if marubozu.is_bullish():
            print("Bullish Marubozu detected")

        # Utility mode
        pattern = CDLMARUBOZU.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLMARUBOZU', max_periods=None):
        """
        Initialize CDLMARUBOZU indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLMARUBOZU using TA-Lib (utility mode).
        ...
        return talib.CDLMARUBOZU(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLMARUBOZU for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Marubozu pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Marubozu pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Marubozu pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
