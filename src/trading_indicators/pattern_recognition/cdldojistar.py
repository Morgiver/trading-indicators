"""
CDLDOJISTAR - Doji Star Candlestick Pattern

Doji Star is a reversal pattern where a Doji gaps away from the previous
candle's body, indicating potential trend reversal or indecision.

Returns:
- 0: Pattern not detected
- 100: Bullish Doji Star
- -100: Bearish Doji Star
"""
...
class CDLDOJISTAR(BaseIndicator):
    """
    CDLDOJISTAR - Doji Star Candlestick Pattern

    A reversal pattern featuring a Doji that gaps away from the previous
    candle, signaling indecision and potential trend change.

    Usage:
        # Auto-sync mode
        doji_star = CDLDOJISTAR(frame=frame)
        if doji_star.is_bullish():
            print("Bullish Doji Star detected")

        # Utility mode
        pattern = CDLDOJISTAR.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLDOJISTAR', max_periods=None):
        """
        Initialize CDLDOJISTAR indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLDOJISTAR using TA-Lib (utility mode).
        ...
        return talib.CDLDOJISTAR(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLDOJISTAR for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Doji Star pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Doji Star pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Doji Star pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
