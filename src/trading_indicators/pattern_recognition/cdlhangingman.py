"""
CDLHANGINGMAN - Hanging Man Candlestick Pattern

Hanging Man is a bearish reversal pattern with a small body at the upper end
and a long lower shadow, appearing after an uptrend.

Returns:
- 0: Pattern not detected
- 100: Bullish Hanging Man
...
class CDLHANGINGMAN(BaseIndicator):
    """
    CDLHANGINGMAN - Hanging Man Candlestick Pattern

    A bearish reversal pattern with a small body and long lower shadow,
    appearing after an uptrend and signaling potential downward reversal.

    Usage:
        # Auto-sync mode
        hanging_man = CDLHANGINGMAN(frame=frame)
        if hanging_man.is_bearish():
            print("Bearish Hanging Man detected")

        # Utility mode
        pattern = CDLHANGINGMAN.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLHANGINGMAN', max_periods=None):
        """
        Initialize CDLHANGINGMAN indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLHANGINGMAN using TA-Lib (utility mode).
        ...
        return talib.CDLHANGINGMAN(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLHANGINGMAN for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Hanging Man pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Hanging Man pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Hanging Man pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
