"""
CDLSTALLEDPATTERN - Stalled Pattern Candlestick Pattern

Stalled Pattern is a bearish reversal pattern with three white candles
showing diminishing momentum, signaling potential trend exhaustion.

Returns:
- 0: Pattern not detected
- 100: Bullish Stalled Pattern
- -100: Bearish Stalled Pattern
"""
...
class CDLSTALLEDPATTERN(BaseIndicator):
    """
    CDLSTALLEDPATTERN - Stalled Pattern Candlestick Pattern

    A bearish reversal pattern with three white candles where momentum
    diminishes, indicating uptrend exhaustion and potential reversal.

    Usage:
        # Auto-sync mode
        stalled_pattern = CDLSTALLEDPATTERN(frame=frame)
        if stalled_pattern.is_bearish():
            print("Bearish Stalled Pattern detected")

        # Utility mode
        pattern = CDLSTALLEDPATTERN.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLSTALLEDPATTERN', max_periods=None):
        """
        Initialize CDLSTALLEDPATTERN indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLSTALLEDPATTERN using TA-Lib (utility mode).
        ...
        return talib.CDLSTALLEDPATTERN(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLSTALLEDPATTERN for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Stalled Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Stalled Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Stalled Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
