"""
CDLTHRUSTING - Thrusting Pattern Candlestick Pattern

Thrusting Pattern is a bearish continuation pattern where a white candle
closes below the midpoint of the previous black candle in a downtrend.

Returns:
- 0: Pattern not detected
- 100: Bullish Thrusting Pattern
- -100: Bearish Thrusting Pattern
"""
...
class CDLTHRUSTING(BaseIndicator):
    """
    CDLTHRUSTING - Thrusting Pattern Candlestick Pattern

    A bearish continuation pattern with a white candle that closes below
    the midpoint of the previous black candle, confirming downtrend continuation.

    Usage:
        # Auto-sync mode
        thrusting = CDLTHRUSTING(frame=frame)
        if thrusting.is_bearish():
            print("Bearish Thrusting Pattern detected")

        # Utility mode
        pattern = CDLTHRUSTING.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLTHRUSTING', max_periods=None):
        """
        Initialize CDLTHRUSTING indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLTHRUSTING using TA-Lib (utility mode).
        ...
        return talib.CDLTHRUSTING(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLTHRUSTING for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Thrusting Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Thrusting Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Thrusting Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
