"""
CDLCLOSINGMARUBOZU - Closing Marubozu Candlestick Pattern

Closing Marubozu is a candle with no shadow at the close, indicating strong
momentum in the direction of the candle (bullish white or bearish black).
...
class CDLCLOSINGMARUBOZU(BaseIndicator):
    """
    CDLCLOSINGMARUBOZU - Closing Marubozu Candlestick Pattern

    A candle with no shadow at the close, showing strong directional momentum
    and conviction in the market direction.

    Usage:
        # Auto-sync mode
        closing_marubozu = CDLCLOSINGMARUBOZU(frame=frame)
        if closing_marubozu.is_bullish():
            print("Bullish Closing Marubozu detected")

        # Utility mode
        pattern = CDLCLOSINGMARUBOZU.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLCLOSINGMARUBOZU', max_periods=None):
        """
        Initialize CDLCLOSINGMARUBOZU indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLCLOSINGMARUBOZU using TA-Lib (utility mode).
        ...
        return talib.CDLCLOSINGMARUBOZU(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLCLOSINGMARUBOZU for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Closing Marubozu pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Closing Marubozu pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Closing Marubozu pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
