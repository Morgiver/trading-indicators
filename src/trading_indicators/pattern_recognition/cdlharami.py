"""
CDLHARAMI - Harami Pattern Candlestick Pattern

Harami Pattern is a reversal pattern where a small candle is contained
within the body of the previous larger candle of opposite color.

Returns:
- 0: Pattern not detected
- 100: Bullish Harami Pattern
- -100: Bearish Harami Pattern
"""
...
class CDLHARAMI(BaseIndicator):
    """
    CDLHARAMI - Harami Pattern Candlestick Pattern

    A reversal pattern with a small candle contained within the previous
    larger opposite-colored candle, indicating indecision and potential reversal.

    Usage:
        # Auto-sync mode
        harami = CDLHARAMI(frame=frame)
        if harami.is_bullish():
            print("Bullish Harami Pattern detected")

        # Utility mode
        pattern = CDLHARAMI.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLHARAMI', max_periods=None):
        """
        Initialize CDLHARAMI indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLHARAMI using TA-Lib (utility mode).
        ...
        return talib.CDLHARAMI(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLHARAMI for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Harami Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Harami Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Harami Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
