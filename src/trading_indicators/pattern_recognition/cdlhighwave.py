"""
CDLHIGHWAVE - High-Wave Candle Candlestick Pattern

High-Wave Candle is a candle with a small body and very long upper and lower
shadows, indicating extreme volatility and market indecision.

Returns:
- 0: Pattern not detected
- 100: Bullish High-Wave Candle
- -100: Bearish High-Wave Candle
"""
...
class CDLHIGHWAVE(BaseIndicator):
    """
    CDLHIGHWAVE - High-Wave Candle Candlestick Pattern

    A candle with a small body and very long shadows in both directions,
    showing extreme volatility and uncertainty in the market.

    Usage:
        # Auto-sync mode
        high_wave = CDLHIGHWAVE(frame=frame)
        if high_wave.is_detected():
            print("High-Wave Candle detected")

        # Utility mode
        pattern = CDLHIGHWAVE.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLHIGHWAVE', max_periods=None):
        """
        Initialize CDLHIGHWAVE indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLHIGHWAVE using TA-Lib (utility mode).
        ...
        return talib.CDLHIGHWAVE(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLHIGHWAVE for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish High-Wave Candle pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish High-Wave Candle pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any High-Wave Candle pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
