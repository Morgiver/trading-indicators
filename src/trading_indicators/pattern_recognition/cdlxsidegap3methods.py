"""
CDLXSIDEGAP3METHODS - Upside/Downside Gap Three Methods Candlestick Pattern

Upside/Downside Gap Three Methods is a continuation pattern where a gap is
followed by a candle that closes the gap, confirming trend continuation.

Returns:
- 0: Pattern not detected
- 100: Bullish Upside Gap Three Methods
- -100: Bearish Downside Gap Three Methods
"""
...
class CDLXSIDEGAP3METHODS(BaseIndicator):
    """
    CDLXSIDEGAP3METHODS - Upside/Downside Gap Three Methods Candlestick Pattern

    A continuation pattern with a gap followed by a candle that closes it,
    confirming the strength and continuation of the prevailing trend.

    Usage:
        # Auto-sync mode
        xside_gap_3methods = CDLXSIDEGAP3METHODS(frame=frame)
        if xside_gap_3methods.is_bullish():
            print("Bullish Upside Gap Three Methods detected")

        # Utility mode
        pattern = CDLXSIDEGAP3METHODS.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLXSIDEGAP3METHODS', max_periods=None):
        """
        Initialize CDLXSIDEGAP3METHODS indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLXSIDEGAP3METHODS using TA-Lib (utility mode).
        ...
        return talib.CDLXSIDEGAP3METHODS(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLXSIDEGAP3METHODS for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Upside Gap Three Methods pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Downside Gap Three Methods pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Gap Three Methods pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
