"""
CDLDOJI - Doji Candlestick Pattern

A Doji is a candlestick pattern where the open and close are virtually equal,
creating a cross-like appearance. It indicates market indecision and potential
reversal points.

Returns:
- 0: Pattern not detected
- 100: Bullish Doji (at support/downtrend)
- -100: Bearish Doji (at resistance/uptrend)
"""
...
class CDLDOJI(BaseIndicator):
    """
    CDLDOJI - Doji Candlestick Pattern

    A Doji forms when open and close prices are nearly equal, indicating
    indecision in the market. Can signal potential reversals.

    Usage:
        # Auto-sync mode
        doji = CDLDOJI(frame=frame)
        if doji.is_bullish():
            print("Bullish Doji detected")

        # Utility mode
        pattern = CDLDOJI.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLDOJI', max_periods=None):
        """
        Initialize CDLDOJI indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLDOJI using TA-Lib (utility mode).
    ...
    def calculate(self, period):
        """Calculate CDLDOJI for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Doji pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Doji pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Doji pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
