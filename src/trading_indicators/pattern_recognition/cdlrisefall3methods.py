"""
CDLRISEFALL3METHODS - Rising/Falling Three Methods Candlestick Pattern

Rising/Falling Three Methods is a continuation pattern with a long candle
followed by three small opposite-colored candles, then another long candle.

Returns:
- 0: Pattern not detected
- 100: Bullish Rising Three Methods
- -100: Bearish Falling Three Methods
"""
...
class CDLRISEFALL3METHODS(BaseIndicator):
    """
    CDLRISEFALL3METHODS - Rising/Falling Three Methods Candlestick Pattern

    A continuation pattern with temporary consolidation candles followed
    by resumption of the trend, confirming trend strength.

    Usage:
        # Auto-sync mode
        rise_fall_3methods = CDLRISEFALL3METHODS(frame=frame)
        if rise_fall_3methods.is_bullish():
            print("Bullish Rising Three Methods detected")

        # Utility mode
        pattern = CDLRISEFALL3METHODS.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLRISEFALL3METHODS', max_periods=None):
        """
        Initialize CDLRISEFALL3METHODS indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLRISEFALL3METHODS using TA-Lib (utility mode).
        ...
        return talib.CDLRISEFALL3METHODS(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLRISEFALL3METHODS for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Rising Three Methods pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Falling Three Methods pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Rising/Falling Three Methods pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
