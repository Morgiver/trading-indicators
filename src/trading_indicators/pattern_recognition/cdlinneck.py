"""
CDLINNECK - In-Neck Pattern Candlestick Pattern

In-Neck Pattern is a bearish continuation pattern where a white candle closes
near the low of the previous black candle in a downtrend.

Returns:
- 0: Pattern not detected
- 100: Bullish In-Neck Pattern
...
class CDLINNECK(BaseIndicator):
    """
    CDLINNECK - In-Neck Pattern Candlestick Pattern

    A bearish continuation pattern with a white candle that closes near
    the low of the previous black candle, confirming downtrend continuation.

    Usage:
        # Auto-sync mode
        in_neck = CDLINNECK(frame=frame)
        if in_neck.is_bearish():
            print("Bearish In-Neck Pattern detected")

        # Utility mode
        pattern = CDLINNECK.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLINNECK', max_periods=None):
        """
        Initialize CDLINNECK indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLINNECK using TA-Lib (utility mode).
        ...
        return talib.CDLINNECK(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLINNECK for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish In-Neck Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish In-Neck Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any In-Neck Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
