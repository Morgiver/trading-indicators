"""
CDLTASUKIGAP - Tasuki Gap Candlestick Pattern

Tasuki Gap is a continuation pattern where a gap is followed by a candle
that partially closes the gap but doesn't fill it completely.

Returns:
- 0: Pattern not detected
- 100: Bullish Tasuki Gap
- -100: Bearish Tasuki Gap
"""
...
class CDLTASUKIGAP(BaseIndicator):
    """
    CDLTASUKIGAP - Tasuki Gap Candlestick Pattern

    A continuation pattern with a gap followed by a candle that partially
    closes but doesn't fill the gap, confirming trend continuation.

    Usage:
        # Auto-sync mode
        tasuki_gap = CDLTASUKIGAP(frame=frame)
        if tasuki_gap.is_bullish():
            print("Bullish Tasuki Gap detected")

        # Utility mode
        pattern = CDLTASUKIGAP.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLTASUKIGAP', max_periods=None):
        """
        Initialize CDLTASUKIGAP indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLTASUKIGAP using TA-Lib (utility mode).
        ...
        return talib.CDLTASUKIGAP(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLTASUKIGAP for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Tasuki Gap pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Tasuki Gap pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Tasuki Gap pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
