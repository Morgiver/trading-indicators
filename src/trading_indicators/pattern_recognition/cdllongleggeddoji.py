"""
CDLLONGLEGGEDDOJI - Long Legged Doji Candlestick Pattern

Long Legged Doji is a Doji with very long upper and lower shadows,
indicating extreme market indecision and potential reversal.

Returns:
- 0: Pattern not detected
- 100: Bullish Long Legged Doji
- -100: Bearish Long Legged Doji
"""
...
class CDLLONGLEGGEDDOJI(BaseIndicator):
    """
    CDLLONGLEGGEDDOJI - Long Legged Doji Candlestick Pattern

    A Doji with very long shadows on both sides, showing intense battle
    between buyers and sellers, often signaling major indecision.

    Usage:
        # Auto-sync mode
        long_legged_doji = CDLLONGLEGGEDDOJI(frame=frame)
        if long_legged_doji.is_detected():
            print("Long Legged Doji detected")

        # Utility mode
        pattern = CDLLONGLEGGEDDOJI.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLLONGLEGGEDDOJI', max_periods=None):
        """
        Initialize CDLLONGLEGGEDDOJI indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLLONGLEGGEDDOJI using TA-Lib (utility mode).
        ...
        return talib.CDLLONGLEGGEDDOJI(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLLONGLEGGEDDOJI for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Long Legged Doji pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Long Legged Doji pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Long Legged Doji pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
