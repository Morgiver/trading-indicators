"""
CDL3OUTSIDE - Three Outside Up/Down Candlestick Pattern

Three Outside Up/Down is a reversal pattern where an engulfing pattern is
confirmed by a third candle that closes above (bullish) or below (bearish).

Returns:
- 0: Pattern not detected
- 100: Bullish Three Outside Up
- -100: Bearish Three Outside Down
"""
...
class CDL3OUTSIDE(BaseIndicator):
    """
    CDL3OUTSIDE - Three Outside Up/Down Candlestick Pattern

    A three-candle reversal pattern with an engulfing pattern confirmed by
    a third candle. Three Outside Up is bullish, Three Outside Down is bearish.

    Usage:
        # Auto-sync mode
        three_outside = CDL3OUTSIDE(frame=frame)
        if three_outside.is_bullish():
            print("Bullish Three Outside Up detected")

        # Utility mode
        pattern = CDL3OUTSIDE.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDL3OUTSIDE', max_periods=None):
        """
        Initialize CDL3OUTSIDE indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDL3OUTSIDE using TA-Lib (utility mode).
        ...
        return talib.CDL3OUTSIDE(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDL3OUTSIDE for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Three Outside Up pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Three Outside Down pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Three Outside pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
