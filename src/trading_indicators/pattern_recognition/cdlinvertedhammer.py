"""
CDLINVERTEDHAMMER - Inverted Hammer Candlestick Pattern

Inverted Hammer is a bullish reversal pattern with a small body at the lower
end and a long upper shadow, appearing after a downtrend.

Returns:
- 0: Pattern not detected
- 100: Bullish Inverted Hammer
- -100: Bearish Inverted Hammer
"""
...
class CDLINVERTEDHAMMER(BaseIndicator):
    """
    CDLINVERTEDHAMMER - Inverted Hammer Candlestick Pattern

    A bullish reversal pattern with a small body and long upper shadow,
    indicating buying pressure and potential upward reversal after downtrend.

    Usage:
        # Auto-sync mode
        inverted_hammer = CDLINVERTEDHAMMER(frame=frame)
        if inverted_hammer.is_bullish():
            print("Bullish Inverted Hammer detected")

        # Utility mode
        pattern = CDLINVERTEDHAMMER.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLINVERTEDHAMMER', max_periods=None):
        """
        Initialize CDLINVERTEDHAMMER indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLINVERTEDHAMMER using TA-Lib (utility mode).
        ...
        return talib.CDLINVERTEDHAMMER(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLINVERTEDHAMMER for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Inverted Hammer pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Inverted Hammer pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Inverted Hammer pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
