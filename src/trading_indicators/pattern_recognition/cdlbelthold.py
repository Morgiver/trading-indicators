"""
CDLBELTHOLD - Belt-hold Candlestick Pattern

Belt-hold is a single candle reversal pattern with a long body opening at the
extreme and closing near the opposite extreme, signaling potential reversal.

Returns:
- 0: Pattern not detected
- 100: Bullish Belt-hold
- -100: Bearish Belt-hold
"""
...
class CDLBELTHOLD(BaseIndicator):
    """
    CDLBELTHOLD - Belt-hold Candlestick Pattern

    A single candle reversal pattern with a strong body opening at one extreme
    and little to no shadow at the opening, indicating strong directional momentum.

    Usage:
        # Auto-sync mode
        belt_hold = CDLBELTHOLD(frame=frame)
        if belt_hold.is_bullish():
            print("Bullish Belt-hold detected")

        # Utility mode
        pattern = CDLBELTHOLD.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLBELTHOLD', max_periods=None):
        """
        Initialize CDLBELTHOLD indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLBELTHOLD using TA-Lib (utility mode).
        ...
        return talib.CDLBELTHOLD(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLBELTHOLD for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Belt-hold pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Belt-hold pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Belt-hold pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
