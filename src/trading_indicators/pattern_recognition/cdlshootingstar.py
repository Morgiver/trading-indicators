"""
CDLSHOOTINGSTAR - Shooting Star Candlestick Pattern

Shooting Star is a bearish reversal pattern with a small body at the lower
end and a long upper shadow, appearing after an uptrend.

Returns:
- 0: Pattern not detected
- 100: Bullish Shooting Star
...
class CDLSHOOTINGSTAR(BaseIndicator):
    """
    CDLSHOOTINGSTAR - Shooting Star Candlestick Pattern

    A bearish reversal pattern with a small body and long upper shadow,
    indicating rejection of higher prices and potential downward reversal.

    Usage:
        # Auto-sync mode
        shooting_star = CDLSHOOTINGSTAR(frame=frame)
        if shooting_star.is_bearish():
            print("Bearish Shooting Star detected")

        # Utility mode
        pattern = CDLSHOOTINGSTAR.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLSHOOTINGSTAR', max_periods=None):
        """
        Initialize CDLSHOOTINGSTAR indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLSHOOTINGSTAR using TA-Lib (utility mode).
        ...
        return talib.CDLSHOOTINGSTAR(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLSHOOTINGSTAR for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Shooting Star pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Shooting Star pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Shooting Star pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
