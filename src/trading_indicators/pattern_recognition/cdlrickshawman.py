"""
CDLRICKSHAWMAN - Rickshaw Man Candlestick Pattern

Rickshaw Man is a Doji with long upper and lower shadows and a small body
in the middle, indicating extreme indecision and potential reversal.

Returns:
- 0: Pattern not detected
- 100: Bullish Rickshaw Man
- -100: Bearish Rickshaw Man
"""
...
class CDLRICKSHAWMAN(BaseIndicator):
    """
    CDLRICKSHAWMAN - Rickshaw Man Candlestick Pattern

    A Doji-like pattern with long shadows and small body centered in the range,
    showing extreme indecision and potential trend reversal.

    Usage:
        # Auto-sync mode
        rickshaw_man = CDLRICKSHAWMAN(frame=frame)
        if rickshaw_man.is_detected():
            print("Rickshaw Man detected")

        # Utility mode
        pattern = CDLRICKSHAWMAN.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLRICKSHAWMAN', max_periods=None):
        """
        Initialize CDLRICKSHAWMAN indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLRICKSHAWMAN using TA-Lib (utility mode).
        ...
        return talib.CDLRICKSHAWMAN(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLRICKSHAWMAN for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Rickshaw Man pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Rickshaw Man pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Rickshaw Man pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
