"""
CDLPIERCING - Piercing Pattern Candlestick Pattern

Piercing Pattern is a bullish reversal pattern where a white candle opens
below and closes above the midpoint of the previous black candle.

Returns:
- 0: Pattern not detected
- 100: Bullish Piercing Pattern
- -100: Bearish Piercing Pattern
"""
...
class CDLPIERCING(BaseIndicator):
    """
    CDLPIERCING - Piercing Pattern Candlestick Pattern

    A bullish reversal pattern with a white candle that opens below and
    pierces deeply into the previous black candle's body.

    Usage:
        # Auto-sync mode
        piercing = CDLPIERCING(frame=frame)
        if piercing.is_bullish():
            print("Bullish Piercing Pattern detected")

        # Utility mode
        pattern = CDLPIERCING.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLPIERCING', max_periods=None):
        """
        Initialize CDLPIERCING indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLPIERCING using TA-Lib (utility mode).
        ...
        return talib.CDLPIERCING(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLPIERCING for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Piercing Pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Piercing Pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Piercing Pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
