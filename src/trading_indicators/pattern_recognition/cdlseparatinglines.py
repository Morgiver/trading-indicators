"""
CDLSEPARATINGLINES - Separating Lines Candlestick Pattern

Separating Lines is a continuation pattern with two opposite-colored candles
that open at the same level, confirming trend continuation.

Returns:
- 0: Pattern not detected
- 100: Bullish Separating Lines
- -100: Bearish Separating Lines
"""
...
class CDLSEPARATINGLINES(BaseIndicator):
    """
    CDLSEPARATINGLINES - Separating Lines Candlestick Pattern

    A continuation pattern with opposite-colored candles opening at the
    same level, confirming the prevailing trend direction.

    Usage:
        # Auto-sync mode
        separating_lines = CDLSEPARATINGLINES(frame=frame)
        if separating_lines.is_bullish():
            print("Bullish Separating Lines detected")

        # Utility mode
        pattern = CDLSEPARATINGLINES.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLSEPARATINGLINES', max_periods=None):
        """
        Initialize CDLSEPARATINGLINES indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLSEPARATINGLINES using TA-Lib (utility mode).
        ...
        return talib.CDLSEPARATINGLINES(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLSEPARATINGLINES for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Separating Lines pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Separating Lines pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Separating Lines pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
