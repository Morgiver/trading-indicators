"""
CDLHOMINGPIGEON - Homing Pigeon Candlestick Pattern

Homing Pigeon is a bullish reversal pattern with two black candles where
the second candle is contained within the body of the first.

Returns:
- 0: Pattern not detected
- 100: Bullish Homing Pigeon
- -100: Bearish Homing Pigeon
"""
...
class CDLHOMINGPIGEON(BaseIndicator):
    """
    CDLHOMINGPIGEON - Homing Pigeon Candlestick Pattern

    A bullish reversal pattern with two black candles where the second,
    smaller black candle is contained within the first, signaling exhaustion.

    Usage:
        # Auto-sync mode
        homing_pigeon = CDLHOMINGPIGEON(frame=frame)
        if homing_pigeon.is_bullish():
            print("Bullish Homing Pigeon detected")

        # Utility mode
        pattern = CDLHOMINGPIGEON.compute(open_prices, high_prices, low_prices, close_prices)
    """

    def __init__(self, frame=None, column_name='CDLHOMINGPIGEON', max_periods=None):
        """
        Initialize CDLHOMINGPIGEON indicator.
    ...
    def compute(open_prices, high_prices, low_prices, close_prices):
        """
        Compute CDLHOMINGPIGEON using TA-Lib (utility mode).
        ...
        return talib.CDLHOMINGPIGEON(open_prices, high_prices, low_prices, close_prices)

    def calculate(self, period):
        """Calculate CDLHOMINGPIGEON for the current period (auto-sync mode)."""
    ...
    def is_bullish(self) -> bool:
        """Check if bullish Homing Pigeon pattern is detected."""
        latest = self.get_latest()
        return latest == 100 if latest is not None else False

    def is_bearish(self) -> bool:
        """Check if bearish Homing Pigeon pattern is detected."""
        latest = self.get_latest()
        return latest == -100 if latest is not None else False

    def is_detected(self) -> bool:
        """Check if any Homing Pigeon pattern is detected (bullish or bearish)."""
        latest = self.get_latest()
